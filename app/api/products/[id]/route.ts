...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  await writeJson('products.json', products.filter((p) => p.id !== params.id))

  const prices = await readJson<SubcontractorProductPrice>('subcontractor_product_prices.json')
  await writeJson('subcontractor_product_prices.json', prices.filter((p) => p.product_id !== params.id))
