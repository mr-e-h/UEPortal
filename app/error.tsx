  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-muted px-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow p-6 text-center space-y-4">
        <h1 className="text-lg font-semibold text-[var(--color-text-primary)]">Noe gikk galt</h1>
        <p className="text-sm text-[var(--color-text-secondary)]">
          En uventet feil oppsto. Prøv igjen — vedvarer det, kontakt admin.
        </p>
        {error.digest && (
          <p className="text-xs text-[var(--color-text-muted)] font-mono">Feil-ID: {error.digest}</p>
        )}
        <div className="flex gap-2 justify-center pt-2">
          ...
          <Link
            href="/"
            className="px-4 py-2 bg-muted text-[var(--color-text-secondary)] text-sm font-medium rounded-lg hover:bg-gray-200"
          >
            Til forsiden
          </Link>
        </div>
