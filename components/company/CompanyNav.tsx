...
/**
 * Company sidebar + mobile drawer, extracted as a client island so the layout
 * can be a server component. Owns drawer state and active-link highlighting.
 * The user name + logout live in the sidebar footer here, so the name is
 * passed in from the server layout.
 */
export default function CompanyNav({ userName }: { userName: string }) {
  ...
  const navContent = (
    <>
      ...
      <div className="flex-none border-t border-border p-4">
        ...
        <LogoutButton className="text-xs text-[var(--color-text-muted)] hover:text-danger mt-0.5 transition-colors" />
      </div>
    </>
  )
