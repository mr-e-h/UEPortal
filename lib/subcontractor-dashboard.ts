...
export interface DashboardProject {
  ...
  project_managers: DashboardProjectManager[]
}

export interface DashboardPendingChangeOrder {
  id: string
  project_id: string
  project_name: string
  project_number: string
...
export interface DashboardRevisionChangeOrder extends DashboardPendingChangeOrder {
  admin_comment: string
}

export interface DashboardPendingWeeklyReport {
  id: string
  project_id: string
  project_name: string
  project_number: string
...
export interface DashboardPayload {
  ...
  pendingChangeOrders: DashboardPendingChangeOrder[]
  revisionChangeOrders: DashboardRevisionChangeOrder[]
  pendingWeeklyReports: DashboardPendingWeeklyReport[]
  projects: DashboardProject[]
}

/**
 * Consolidated dashboard payload for one subcontractor. Single source of the
 * dashboard joins/sums, shared by the RSC dashboard page (server-side, subId
 * from the session) and the GET /api/subcontractor/dashboard route (which keeps
 * an identical payload for the mobile/nav clients). The caller is responsible
 * for resolving `subId` from the effective session — this function never reads
 * the URL and never selects customer-price fields.
 *
 ...
 */
export async function getSubcontractorDashboard(subId: string): Promise<DashboardPayload> {
  ...
  // KPI 3 — fakturerbart. Bruk readyToInvoice som ENESTE kilde for formelen
  // (approvedWork + approvedChangeOrders − invoiced). Klampes ≥ 0 her som før
  // (faktura kan av og til løpe forbi rapportene); helperen klamper ikke selv.
  const fakturerbart = Math.max(
    0,
    readyToInvoice({
      approvedWork: approvedLineValue,
