...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 32px auto; padding: 0 16px; color: #0F172A;">
  <div style="border-top: 4px solid #2563EB; padding-top: 24px;">
...
Hvis du ikke forventet denne invitasjonen kan du ignorere e-posten.`
  ...
  const html = baseHtml(
    ...
    <p style="margin: 24px 0;">
      <a href="${escapeHtml(acceptUrl)}" style="display: inline-block; background: #2563EB; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 500; font-size: 14px;">Aksepter invitasjon</a>
    </p>
...
Hvis du ikke ba om dette, kan du ignorere e-posten. Passordet ditt forblir uendret.`
  ...
  const html = baseHtml(
    ...
    <p style="margin: 24px 0;">
      <a href="${escapeHtml(resetUrl)}" style="display: inline-block; background: #2563EB; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 500; font-size: 14px;">Velg nytt passord</a>
    </p>
